...
import type { RoleDto } from '@/api/generated/types.gen';
...
export default function RoleList() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['roleList'] });
      queryClient.invalidateQueries({ queryKey: ['roleDetail', viewId] });
      setDrawerMode('view');
    },
  ...
  return (
    <div className="flex flex-col h-full bg-[#141414] p-4 rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <Title level={3} className="!m-0 text-gray-100">角色管理</Title>
        <Space>
          <Button
            type="default"
            icon={<SearchOutlined />}
      ...
      <Drawer
        title={drawerMode === 'create' ? '新增角色管理' : drawerMode === 'edit' ? '編輯角色管理' : '檢視角色管理'}
        width={720}
        size="large"
        onClose={closeDrawer}
        open={isDrawerOpen}
