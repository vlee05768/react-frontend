...
import type { EmployeeDto } from '@/api/generated/types.gen';
...
export default function EmployeeList() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['employeeList'] });
      queryClient.invalidateQueries({ queryKey: ['employeeDetail', viewId] });
      setDrawerMode('view');
    },
  ...
  const columns = [
    {
      ...
      render: (_: any, record: EmployeeDto) => (
        <Space size="small">
          ...
          {hasPermission('BasicData.Employees.Update') && (
            <Tooltip title="編輯">
              <Button
                type="text"
                icon={<EditOutlined />}
  ...
  return (
    <div className="flex flex-col h-full bg-[#141414] p-4 rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <Title level={3} className="!m-0 text-gray-100">員工基本檔</Title>
        <Space>
          <Button
            type="default"
            icon={<SearchOutlined />}
      ...
      <Drawer
        title={drawerMode === 'create' ? '新增員工基本檔' : drawerMode === 'edit' ? '編輯員工基本檔' : '檢視員工基本檔'}
        width={720}
        size="large"
        onClose={closeDrawer}
        open={isDrawerOpen}
