...
import type { MachineDto } from '@/api/generated/types.gen';
...
export default function MachineList() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['machineList'] });
      queryClient.invalidateQueries({ queryKey: ['machineDetail', viewId] });
      setDrawerMode('view');
    },
  ...
  const openCreate = () => {
    navigate('/production-quality/machines/new');
  ...
  const openView = (code: string | number) => {
    navigate(`/production-quality/machines/${code}`);
    setDrawerMode('view');
  };
  ...
  const columns = [
    {
      ...
      width: 120,
      fixed: 'left' as const,
      render: (_: any, record: MachineDto) => (
        <Space size="small">
          <Tooltip title="檢視">
            <Button
              type="text"
          ...
          {hasPermission('ProductionQuality.Machines.Update') && (
            <Tooltip title="編輯">
              <Button
                type="text"
                icon={<EditOutlined />}
  ...
  return (
    <div className="flex flex-col h-full bg-[#141414] p-4 rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <Title level={3} className="!m-0 text-gray-100">機台管理</Title>
        <Space>
          <Button
            type="default"
            icon={<SearchOutlined />}
      ...
      <Drawer
        title={drawerMode === 'create' ? '新增機台管理' : drawerMode === 'edit' ? '編輯機台管理' : '檢視機台管理'}
        width={720}
        size="large"
        onClose={closeDrawer}
        open={isDrawerOpen}
